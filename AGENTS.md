...
- `Smartitecture.Tests/`: xUnit tests (primary). Legacy mirror under `tests/Smartitecture.Tests/`.
...
- Run tests + coverage: `dotnet test --collect:"XPlat Code Coverage"`
...
## Testing Guidelines
- Framework: xUnit with `coverlet.collector`. Place tests in `Smartitecture.Tests/` mirroring namespaces/folders.
- Test names: `Class_Method_ShouldBehavior`. Prefer service/command tests over UI. Mock external calls.
- Run `dotnet test` locally before PRs; target meaningful coverage on Services and Commands.
